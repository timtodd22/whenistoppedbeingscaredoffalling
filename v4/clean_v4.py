"""v4: caption removal with Real-ESRGAN on the refilled box (see clean.py for the method).
...
SR_Y0, SR_Y1 = 630, 792                  # source rows covering the box plus its feather (output ~1416-1780)
...
def clean_frame(g, fin):
    ...
    # Real-ESRGAN on just the band that gets pasted in, when it has texture worth recovering
    band = cand[SR_Y0:SR_Y1]
    if cv2.Laplacian(cv2.cvtColor(band, cv2.COLOR_RGB2GRAY), cv2.CV_32F).var() > 12:
        from esrgan import upscale4
        y0, y1 = round(SR_Y0 * H / SH), round(SR_Y1 * H / SH)
        sr = cv2.resize(upscale4(band.copy()), (W, y1 - y0), interpolation=cv2.INTER_AREA)
        u[y0:y1] = cv2.addWeighted(sr, 0.7, u[y0:y1], 0.3, 0)
...
def work(args):
    ...
    enc = subprocess.Popen([FF, '-loglevel', 'error', '-y', '-f', 'rawvideo', '-pix_fmt', 'rgb24',
                            ...
                            '-crf', '12', '-pix_fmt', 'yuv420p', f'parts_v4/p{f0:04d}.mp4'], stdin=subprocess.PIPE)
    errs = []
...
if __name__ == '__main__':
    ...
    json.dump(errs, open('clean_errs_v4.json', 'w'))
    open('parts_v4/list.txt', 'w').write(''.join(f"file 'p{a:04d}.mp4'\n" for a, _ in chunks))
