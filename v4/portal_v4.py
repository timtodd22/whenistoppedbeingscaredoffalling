"""v4 (Real-ESRGAN upscaling). Replace the fire-portal transition (frames 152-181) with raw source:
...
def up(x):
    ...
    lz = cv2.resize(x, (W, H), interpolation=cv2.INTER_LANCZOS4)
    lz = cv2.addWeighted(lz, 1.5, cv2.GaussianBlur(lz, (0, 0), 1.2), -0.5, 0)
    sr = cv2.resize(upscale4(np.ascontiguousarray(x)), (W, H), interpolation=cv2.INTER_AREA)
    return cv2.addWeighted(sr, 0.7, lz, 0.3, 0)
...
np.save('portal_clean_v4.npy', np.array(out))
...
cv2.imwrite('portal_v4.jpg', cv2.cvtColor(np.concatenate(t, 1), cv2.COLOR_RGB2BGR))
