"""v4 render (on conform_v4.mp4): v3's look and camera plus depth-occluded, voice-reactive lyrics,
the a cappella bridge treatment, and the longer ending with a fade.
...
from plan_v4 import CUTS, TRANS, FLASH, COLD, DIVE, CHORUS, DROP, END, N, FPS, ACAPPELLA, section_intensity
import lyrics_v4

FF = '/usr/local/lib/python3.11/dist-packages/imageio_ffmpeg/binaries/ffmpeg-linux-x86_64-v7.0.2'
SRC, AUDIO = 'conform_v4.mp4', 'audio_v4.m4a'
W, H = 1080, 1920
AX, AY = 540.0, 1100.0
...
def intensity(t):
    return 0.15 if ACAPPELLA[0] <= t < ACAPPELLA[1] else section_intensity(t)


def beat_env(t, tau):
    if ACAPPELLA[0] <= t < ACAPPELLA[1]:                      # no drums under the a cappella voice
        return 0.0
...
GRAIN_AMT = float(os.environ.get('GRAIN', '1.6'))   # 1.6 = upload master; the v3 preview used ~3.5
...
def look(img, t, I, gi):
    ...
    f *= VIG
    f += GRAIN[gi % 8][..., None] * GRAIN_AMT * (1.0 + 1.0 * (1 - np.abs(2 * L - 1)))
...
def render_frame(img, gi, si, a, b):
    ...
    if gi >= END:
        ...
        if emb is not None:
            out += emb * 0.8
        fade = min(1.0, (gi - END) / 6) * min(1.0, (N - 1 - gi) / 57)
        return (LYR.draw(np.clip(out, 0, 255).astype(np.uint8), t, 0.0, 0.0).astype(np.float32) * fade).astype(np.uint8)
    u = (gi - a) / max(1, b - a - 1)
    since = gi - a
    ...
    out = look(out, t, I, gi)
    if ACAPPELLA[0] <= t < ACAPPELLA[1]:
        k = min(1.0, (t - ACAPPELLA[0]) / 0.3) * min(1.0, (ACAPPELLA[1] - t) / 0.15)
        lum = (out.astype(np.float32) @ np.array([0.299, 0.587, 0.114], np.float32))[..., None]
        out = np.clip(out * (1 - 0.35 * k) + lum * 0.35 * k - 6 * k, 0, 255).astype(np.uint8)
