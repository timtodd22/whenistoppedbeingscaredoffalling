"""Beat-synced kinetic re-edit for TikTok.
...
SRC, AUDIO = 'video.mp4', 'TheDayIStoppedBeingScaredOfFalling_audio_-14LUFS.m4a'
W, H, FPS, N = 1080, 1920, 30.0, 1987
AX, AY = 540.0, 1100.0                      # transform anchor, above the caption band
cv2.setNumThreads(4)

CUTS = [52, 83, 122, 254, 271, 379, 447, 462, 493, 523, 621, 660, 681, 704, 744, 770, 780,
...
def ramp_ok(a, b):
    ...
    if b - a < 24 or intensity((ta + tb) / 2) < 0.6 or b > END_CARD:
        return False
    return not any(ta + 0.1 < c < tb - 0.1 for c in CAPTION_CHANGES)
...
def radial_blur(img, amt):
    ...
    for _ in range(5):
        step /= 2
        M = cv2.getRotationMatrix2D((AX / q, AY / q), 0, 1 + step)
...
def render_frame(img, gi, shot_idx, a, b):
    ...
    z = 1.0 + (z - 1.0) if z < 1.12 else 1.12 + 0.3 * (z - 1.12)   # soft cap keeps captions in frame
    ...
    M = cv2.getRotationMatrix2D((AX, AY), rot, z)
    M[0, 2] += tx; M[1, 2] += ty
