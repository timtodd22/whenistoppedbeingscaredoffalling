...
W,H=1080,1920
portal=np.load('portal_clean.npy')
dec=subprocess.Popen([FF,'-loglevel','error','-f','concat','-safe','0','-i','parts/list.txt','-f','rawvideo','-pix_fmt','rgb24','-'],stdout=subprocess.PIPE)
enc=subprocess.Popen([FF,'-loglevel','error','-y','-f','rawvideo','-pix_fmt','rgb24','-s',f'{W}x{H}','-r','30','-i','-',
    ...
    '-crf','14','-pix_fmt','yuv420p','-c:a','copy','-shortest','-movflags','+faststart','clean_master.mp4'],stdin=subprocess.PIPE)
n=0
while True:
